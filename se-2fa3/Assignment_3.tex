...
\be
  ...
  \begin{itemize}
    ...
    \item $\mname{suc}(x) = y$ where $x, y \in \Sigma^*$ states that the word y is immediately after the word x. For example, $\mname{suc}(sun) = day$ would be describing $sunday$.
    \item $\mname{pred}(x) = y$ where $x, y \in \Sigma^*$ states that the word y is immediately before the word x. For example, $\mname{pred}(sun) = day$ would be describing $daysun$.
  ...
  \be
    ...
    $\ForallApp x \mdot (a(1) \lor b(1) \lor c(1)) \land (\mname{last}(x) \rightarrow a(x)) \lor \mname{last}(x) \rightarrow b(x)) \lor \mname{last}(x) \rightarrow c(x))$

    \item $\set{uaav \mid u, v \in \Sigma^*}$.\\
    $\ForallApp u, v \in \Sigma^* \mdot \mname{between}(u, v) = aa \land \mname{suc}(v)=\epsilon \land \mname{pred}(u)=\epsilon$
    
    \item $\set{uavbw \mid u,v,w \in \Sigma^*}$.\\
    $\ForallApp u, v, w \in \Sigma^* \mdot \mname{between}(u, v) = a \land \mname{between}(v, w) = b \land \mname{suc}(w)=\epsilon \land \mname{pred}(u)=\epsilon$
